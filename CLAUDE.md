...
tui.Dockerfile       cs_tui image (multi-stage golang -> scratch); runs with --network=none, sock-only
tui/                 Go (Bubble Tea) TUI source (main.go, model.go, view.go, daemon.go, plugins.go, markdown.go)
tui/go.mod tui/go.sum  pinned dep tree (committed; generated by `go mod tidy`)
