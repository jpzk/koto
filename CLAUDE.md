...
- **Edits to `sidecar/entrypoint.sh` are also live.** The daemon bind-mounts `sidecar/entrypoint.sh` into each sidecar at `/e.sh:ro`. Edits are picked up on the next sidecar respawn (`/new <g>`, or `make clean && make host-run`). Image rebuild (`make build`) is only needed when changing `sidecar/Dockerfile` itself or upgrading the `claude-code` npm package.
