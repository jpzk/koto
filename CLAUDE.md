...
- **The Go toolchain itself is the one exception to the six-week lag besides Firecracker, for the same reason.** Go patch releases ARE the security fixes for the standard library every binary here links, and a Go minor series stops receiving them once it is two behind (1.24 died when 1.26 shipped) — so `GO_IMAGE` (Makefile + `fcguest/build-rootfs.sh`), the `toolchain` line in every `go.mod`/`go.work`, and the CI `go-version` track the NEWEST patch of a supported series and move together (all 1.25.14 as of 2026-09-05, image from 2026-08-19). The ordinary rule still applies to every module pin.
...
- **CI is exactly one GitHub Action: govulncheck on the `release` branch** (`.github/workflows/govulncheck.yml`, also runnable by hand via workflow_dispatch). govulncheck checks the Go vulnerability DB against each module's CALL GRAPH, so it reports only reachable vulnerabilities, standard library included; it needs network, so it is not a pre-commit hook. It runs on `release`, not `main`, because it is a release gate: the moment to decide whether a pinned dependency's six-week soak must be cut short for a reachable vulnerability. The Go version is pinned to the one the release binaries are built with (Makefile `GO_IMAGE`) so stdlib findings describe the shipped bytes — bump the two together. Actions pinned by SHA, govulncheck by version, all ≥6 weeks old. **First run (2026-09-05, govulncheck v1.6.0) was red in every module** — daemon 15 reachable, tui 17, fcguest/protocol 13 under the then-shipped go1.24.13 — and was fixed the same day by moving to **Go 1.25.14** (Go 1.24 fell out of support when 1.26 shipped, so 13 reachable stdlib findings were fixable only by a toolchain bump; see the Go-toolchain exception under Conventions) and bumping grpc v1.80.0→v1.82.1, x/net v0.49.0→v0.57.0, x/text v0.33.0→v0.40.0, x/crypto v0.47.0→v0.54.0 and goldmark v1.7.13→v1.7.17, every new pin ≥6 weeks old (audited: all 64 pins across the four modules pass). **Two reachable findings remain, deliberately**: GO-2026-6354/6355 (DoS in `x/crypto/ssh` channels, reached from gvisor-tap-vsock's `virtualnetwork.New` in `fcnet.go`) are fixed only in x/crypto v0.56.0, released 2026-09-02 — inside the six-week window until **2026-10-14**, when the bump becomes eligible. Until then the release branch reports exactly those two.
