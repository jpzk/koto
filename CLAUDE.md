...
make assets     # fetch firecracker (pinned v1.17.0) + BUILD the guest kernel (kernel;
...
- **Streaming events come from a daemon-side log tailer, not from the TUI.** The daemon runs one tailer (`tailFrom`, `logtail.go`) per stream — the group log plus each concurrency slot's `log.<n>`, started by `ensureTail`/`ensureSlotTail`, which open and position the file BEFORE returning so a turn's `[[session]]` marker written right after is never missed; it parses the marker grammar (`>>> ` = prompt, otherwise = response) and fans event frames out to all subscribers. The TUI therefore needs no filesystem access to a group's workspace.
