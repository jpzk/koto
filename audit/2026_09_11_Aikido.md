...
| **Triage** | `docs/history/SECURITY_11_SEP.md`, the per-finding ledger this file is compiled from. Work ran from `66806fb` to `75e5930`, 144 commits, all on 2026-09-11. |
...
|---|---|---|---|---|---|
| H1 | high | Legacy workspace.img symlink enables cross-boundary VM disk redirection | `daemon/fc.go` | fixed | `66806fb` |
| H2 | high | Jailed Firecracker can impersonate its guest on the privileged host control plane | `daemon/fc.go` | accepted | `66806fb` |
| M2 | medium | Unauthenticated local vsock peers can invoke the guest agent as root | `fcguest/main.go` | fixed (defense in depth) | `850b4e3` |
| M3 | medium | DNS rebinding bypasses the HTTP egress destination policy | `daemon/proxy.go` | fixed | `7ffb36a` |
| M4 | medium | AttachShell permits cross-session tmux attachment within an authorized group | `daemon/grpc_server.go` | fixed (hygiene), claim partly rejected | `850b4e3` |
| M5 | medium | Security-sensitive helpers are resolved from an untrusted PATH | `daemon/install.go` | not a finding | `4ffdf3b` |
| M6 | medium | IPv6 AWS metadata endpoint bypasses the guest egress filter | `daemon/fcnet.go` | fixed | `7ffb36a` |
| M7 | medium | Group-scoped Config permission bypasses field-level posture authorization | `daemon/grpc_server.go` | fixed | `850b4e3` |
| M8 | medium | `koto tui` executes an attacker-selected executable with administrator credentials | `daemon/tui_cmd.go` | not a finding | `4ffdf3b` |
| M9 | medium | Peer reports are promoted into MAIN's executable conversation | `daemon/report.go` | accepted | `245c578` |
| M10 | medium | Independent DNS lookups bypass the boot-time egress profile | `daemon/proxy.go` | fixed | `7ffb36a` |
| M11 | medium | Unvalidated cs-job IDs enable path traversal outside the jobs directory | `sidecar/cs-job` | fixed (hygiene) | `4ffdf3b` |
| M12 | medium | Claude worker permanently bypasses all permission checks | `fcguest/turn.go` | accepted | `4ffdf3b` |
| M13 | medium | Root agent follows worker-controlled temporary-file symlinks | `fcguest/main.go` | fixed | `4ffdf3b`, `f89bc60` |
| M14 | medium | Destroyed groups retain executable schedules and can resurrect a reused group | `daemon/groups.go` | fixed | `4ffdf3b` |
| M15 | medium | Proxy concurrency limits do not impose an aggregate request-body memory budget | `daemon/proxy.go` | fixed | `c6631c8` |
| M16 | medium | Concurrent whole-file config updates can restore revoked group security posture | `daemon/config.go` | fixed | `63ef84c` |
| M17 | medium | Send and schedule paths can provision arbitrary unregistered groups | `daemon/send.go` | fixed | `63ef84c` |
| M18 | medium | List and WatchState bypass group ACLs and disclose unauthorized job metadata | `daemon/grpc_server.go` | fixed | `c6631c8` |
| M19 | medium | Stale background tailer writes into a reused slot and misattributes output to a new session | `daemon/send.go` | fixed | `c6631c8` |
| M20 | medium | Main-to-peer goals can bypass the mandatory human-approval gate | `daemon/goals.go` | fixed, partly rejected | `63ef84c` |
| M21 | medium | Turn text marker escaping is bypassable across frame boundaries | `daemon/fcturn.go` | fixed | `573ec57` |
| M22 | medium | Guest-facing framed channels permit resource exhaustion with incomplete frames | `daemon/fcframe.go` | fixed | `3a7d576` |
| M23 | medium | Privileged job readers follow worker-controlled symlinks | `fcguest/main.go` | fixed, by removing the privilege | `573ec57` |
| M24 | medium | Recursive shared root mount allows worker mount events to propagate into fc-agent | `fcguest/main.go` | accepted | `1b843ee` |
| M25 | medium | Long-lived gRPC streams retain access after authorization revocation | `daemon/auth.go` | fixed | `1b843ee` |
| M26 | medium | Oversized cron step can crash the daemon via integer wraparound | `daemon/cron.go` | fixed | `573ec57` |
| M27 | medium | Guest-facing vsock listeners allow unbounded resource retention | `daemon/fc.go` | fixed | `3a7d576` |
| M28 | medium | Group-scoped Send authorization permits cross-session conversation hijacking | `daemon/grpc_server.go` | not a finding | `245c578` |
| M29 | medium | Workflow requests can provision unregistered groups without spawn authorization | `daemon/grpc_server.go` | fixed | `245c578` |
| M30 | medium | Group-scoped Metrics RPC discloses another group's latest metric | `daemon/grpc_server.go` | fixed | `245c578` |
| M31 | medium | Goal dispatch proceeds after failed conversation reset | `daemon/goals.go` | fixed | `4c4f685` |
| M32 | medium | Install migration resurrects revoked client credentials from stale clone | `daemon/install.go` | fixed | `8bc306f` |
| M33 | medium | Provider-controlled session ID enables privileged path traversal during session clearing | `daemon/groups.go` | fixed | `245c578` |
| M34 | medium | Host-address isolation fails open on stale or failed self-address refresh | `daemon/fcnet.go` | fixed | `245c578` |
| M35 | medium | Empty or null ACL targets authorize global cross-group reads | `daemon/acl.go` | fixed | `a40e24f` |
| M36 | medium | Group-wide attachment spool can deliver orphaned uploads to unrelated turns | `daemon/fc.go` | fixed | `4540c9c` |
| M37 | medium | gRPC lifecycle paths bypass the daemon-wide group quota | `daemon/groups.go` | fixed (with M17/M29) | `a40e24f` |
| M38 | medium | gRPC GoalSet bypasses the protected main-group autonomous-goal restriction | `daemon/grpc_server.go` | fixed | `a40e24f` |
| M39 | medium | Daemon-writable TUI path enables symlink redirection into operator-owned files | `tui/persist.go` | fixed | `a40e24f` |
| M40 | medium | Slot lifecycle races allow stale cleanup and VM-exit timing to corrupt or strand allocations | `daemon/queue.go` | fixed | `4c4f685` |
| M41 | medium | IPv4-mapped IPv6 loopback destinations bypass the host control-plane filter | `daemon/fcnet.go` | not a finding (pinned by a test) | `243418f` |
| M42 | medium | Guest-controlled job_done callbacks can inject turns into same-group sessions and exhaust notification state | `daemon/ctl.go` | fixed (the two halves that matter) | `1996e2a` |
| M43 | medium | Installer executes a PATH-resolved `git` after privileged operations | `daemon/setup.go` | not a finding | `2f618a4` |
| M44 | medium | Goal session namespace collision permits cross-goal state and execution interference | `daemon/goals.go` | fixed | `07118a3` |
| M45 | medium | Named Claude session state is readable and writable by every worker | `fcguest/turn.go` | not a finding | `2f618a4` |
| M46 | medium | Mutable package resolution can poison the shared golden rootfs during a root build | `fcguest/Dockerfile.rootfs` | partly fixed | `b8b4b67` |
| M47 | medium | Concurrent GoalSet requests can allocate the same run name | `daemon/goals.go` | fixed | `b26b35d` |
| M48 | medium | Unbounded streamed transcript blocks permit daemon and TUI resource exhaustion | `daemon/logparse.go` | fixed | `b6ed8e3` |
| M49 | medium | Guest ctl plane can approve delegated goals without human authorization | `daemon/ctl.go` | fixed | `1be38cb` |
| M50 | medium | Destroyed group goals remain readable after group-name reuse | `daemon/grpc_server.go` | fixed | `c014248` |
| M51 | medium | Unbounded Firecracker console output can exhaust the host filesystem | `daemon/fc.go` | fixed | `8f9c540` |
| M52 | medium | Symlinked group state escapes the trusted state root during VM staging | `daemon/fc.go` | not a finding (with one part already fixed) | `c4de371` |
| M53 | medium | Unbound guest turn streams let a compromised guest spoof turn lifecycle | `daemon/fcturn.go` | accepted | `c4de371` |
| M54 | medium | Authorized Send callers can exhaust daemon resources with unbounded session creation | `daemon/send.go` | fixed | `fb28c51` |
| M55 | medium | Concurrent attachment uploads bypass the per-group pending-disk quota | `daemon/attachments.go` | fixed | `e1b2ff0` |
| M56 | medium | Guest-controlled job metadata can inject model notifications and host prompt logs | `daemon/notify.go` | fixed | `dffcb8a` |
| M57 | medium | Non-admin ACL grants can stop the protected main orchestrator | `daemon/grpc_server.go` | not a finding | `c4de371` |
| M58 | medium | Prompt data is inherited by Venice workspace commands | `sidecar/venice_stream.js` | fixed (with a named residual) | `8401902`, `9527c24` |
| M59 | medium | Stale Firecracker reaper corrupts replacement VM state after restart | `daemon/fc.go` | fixed | `742b69c` |
| M60 | medium | Clear reports success without fencing queued and active work | `daemon/groups.go` | fixed | `a7d315e` |
| M61 | medium | Cross-session stream confusion after ambiguous agent delivery | `daemon/queue.go` | fixed | `280b83c` |
| M62 | medium | Legacy JobTail raw frames allow terminal escape injection | `tui/daemon.go` | fixed | `aca7432` |
| M63 | medium | Stop/Destroy can be bypassed by work admitted across the group lifecycle barrier | `daemon/queue.go` | fixed | `d08011e` |
| M64 | medium | Transcript clear does not invalidate in-memory replay and subscriber delivery | `daemon/events.go` | fixed | `5b4d621` |
| M65 | medium | Group-only report capability accepts stale or canceled delegation results | `daemon/report.go` | partly fixed, partly rejected | `5db9fe0` |
| M66 | medium | Untrusted goal and agent output is replayed as trusted instructions across agent sessions | `daemon/goals.go` | fixed for the agent-authored fields | `7977c93` |
| M67 | medium | Unvalidated installer values allow systemd unit and EnvironmentFile injection | `daemon/install.go` | fixed | `d744e35` |
| M68 | medium | Stale Firecracker PID can be reused to kill or misidentify another process | `daemon/fc.go` | fixed | `24e962b` |
| M69 | medium | Parsed JobTail retains unbounded open blocks in the daemon | `daemon/logparse.go` | already fixed (M48) | `8e8e522` |
| M70 | medium | Unbounded send payloads exhaust daemon memory through per-session queues | `daemon/queue.go` | fixed | `8e8e522` |
| M71 | medium | Pre-queue attachment persistence can orphan uploads and reuse them in later turns | `daemon/grpc_server.go` | already fixed (M36) | `8e8e522` |
| M72 | medium | Credential and private-key files retain or inherit non-private permissions | `daemon/install.go` | fixed (the migration path); the rest declined with reasons | `cbc2c66` |
| M73 | medium | JSON-escaped terminal controls bypass event sanitization in tool rendering | `tui/view.go` | fixed | `6ae0131` |
| M74 | medium | Guest-controlled daemon error text reaches terminal output without sanitization | `daemon/grpc_server.go` | fixed | `2655fb9` |
| M75 | medium | Authenticated senders can exhaust daemon resources with unbounded session creation | `daemon/queue.go` | fixed | `fb28c51` |
| M76 | medium | Background subagents omit the harness system prompt | `sidecar/cs-job` | fixed | `4d55057` |
| M77 | medium | Installer accepts attacker-controlled descendant symlinks in the state directory | `daemon/install.go` | not a finding | `2655fb9` |
| M78 | medium | Authenticated JobTail streams can exhaust daemon and guest resources | `daemon/grpc_server.go` | fixed | `fbde6a6` |
| M79 | medium | Untrusted background-task paths are opened by the guest-root agent | `daemon/send.go` | premise removed by M23 | `c3d7c14` |
| M80 | medium | Authorized job-observability RPCs permit unbounded guest-agent execution | `daemon/grpc_server.go` | fixed (concurrency); context propagation declined | `e00a34d` |
| M81 | medium | Concurrent session clearing can discard active log writes and misattribute later output | `daemon/sessions.go` | fixed | `4ecd99a` |
| M82 | medium | Cumulative partial-line events amplify guest output into resource exhaustion | `daemon/logtail.go` | fixed | `9339e4b` |
| M83 | medium | Detached jobs can outlive turns and exhaust persistent workspace resources | `sidecar/cs-job` | partly fixed, partly by design | `c3d7c14`, `d9f8ae7` |
| M84 | medium | Restart authorization can register arbitrary new groups | `daemon/groups.go` | already fixed (M17) | `c3d7c14` |
| M85 | medium | Interrupt can cancel a subsequent session turn via a TOCTOU race | `daemon/queue.go` | fixed | `cbefb99` |
| M86 | medium | `koto ctl shell` forwards guest terminal-control bytes to the operator terminal | `daemon/ctl_cli.go` | fixed | `ee09512` |
| M87 | medium | SchedAdd permits unbounded persistent schedule growth across group names | `daemon/grpc_server.go` | fixed | `c6b8f01` |
| M88 | medium | Preflight remediation makes /dev/kvm globally writable | `daemon/setup_checks.go` | fixed | `bf436c9` |
| M89 | medium | Concurrent group recreation during destroy can corrupt replacement VM state | `daemon/groups.go` | fixed | `6c7bf6d` |
| M90 | medium | Unbounded Venice transcript can exhaust a session workspace and deny turns | `sidecar/venice_stream.js` | fixed | `66c4b78` |
| M91 | medium | Clearing the replacement network policy leaves legacy WAN egress active | `daemon/groups.go` | fixed | `c018e5f` |
| M92 | medium | Guest disconnects do not cancel credentialed upstream proxy work | `daemon/proxy.go` | fixed | `3a22ef6` |
| M93 | medium | Claude executable parent directories bypass the daemon's ProtectHome boundary | `daemon/install.go` | fixed | `4f50770` |
| M94 | medium | Age-trimmed partial events remain pinned by ringPartial | `daemon/events.go` | fixed | `ab95838` |
| M95 | medium | Authorized clients can exhaust daemon capacity with unbounded streaming RPCs | `daemon/auth.go` | fixed | `e6ecef3` |
| M96 | medium | TUI lifecycle commands omit the selected goal name | `tui/daemon.go` | fixed | `ca468e9` |
| M97 | medium | ctl ask can substitute another session's response | `daemon/ctl_cli.go` | fixed for the cross-session case; the same-session case named | `aa35cc7` |
| M98 | medium | Destroyed group job cache is reused after same-name recreation | `daemon/groups.go` | fixed | `3609e2b` |
| M99 | medium | Unbounded token-rate sample retention enables daemon resource exhaustion | `daemon/tokrate.go` | fixed | `7e94691` |
| M100 | medium | Fallback scanner container can read and exfiltrate the entire checkout | `tools/hooks/pre-commit` | fixed | `761922a` |
| M101 | medium | Concurrent ctl spawn requests bypass the group-count cap | `daemon/ctl.go` | fixed | `78d76c5` |
| M102 | medium | Plan phase treats aborted or self-healed turns as completed | `daemon/goals.go` | fixed | `833e152` |
| M103 | medium | Unbounded guest-to-egress proxy tunnels enable shared resource exhaustion | `daemon/proxy.go` | fixed (admission); idle timeouts declined | `c3473f4` |
| M104 | medium | Notification and tailer state survives group destruction and name reuse | `daemon/logtail.go` | fixed | `8482460` |
| M105 | medium | Tool transcript renders unredacted tool arguments | `tui/model.go` | not a finding | `6b9e699` |
| M106 | medium | Group-scoped interrupt authorization permits cross-session turn cancellation | `daemon/grpc_server.go` | not a finding | `6b9e699` |
| M107 | medium | Sensitive prompts are persisted in job command metadata | `sidecar/cs-job` | accepted | `0d467b8` |
| M108 | medium | Makefile PKI generation leaves private keys readable by other local users | `Makefile` | fixed | `fdcf716` |
| M109 | medium | Worker-controlled status object can wedge Firecracker VM initialization | `fcguest/turn.go` | fixed | `5637894` |
| M110 | medium | Memory admission can be bypassed by racing mutable VM configuration | `daemon/fc.go` | fixed | `57d9ae2` |
| M111 | medium | Untrusted TUI content can inject terminal control sequences | `tui/theme.go` | fixed for the reachable class; the blanket render-boundary sanitizer declined | `526a07c` |
| M112 | medium | Purge can be redirected by a path-substitution race | `daemon/uninstall.go` | fixed | `0d467b8` |
| M113 | medium | Transcript files are created with permissions readable by other local users | `daemon/logtail.go` | fixed | `af88cc7` |
| M114 | medium | Published-port forwarding permits unbounded idle connection exhaustion | `daemon/fc.go` | already fixed (M22/M27) | `af88cc7` |
| M115 | medium | Unsolicited guest report requests trigger repeatable host-daemon resource consumption | `daemon/ctl.go` | fixed | `16848eb` |
| M116 | medium | Installer promotes unverified Firecracker runtime and guest assets | `daemon/install.go` | fixed | `681f42a` |
| M117 | medium | Pending upload quota permits oversized host-to-guest frames and wedges group delivery | `daemon/fc.go` | fixed | `3c925b7` |
| M118 | medium | PID 1 follows a worker-controlled `/workspace/.cs` symlink during privileged setup | `fcguest/main.go` | fixed | `46ee238` |
| M119 | medium | Guest-triggered proxy errors can grow the host group log without bound | `daemon/proxy.go` | fixed | `9ccfe08` |
| M120 | medium | Worker-controlled `ctl.out` FIFO can block the guest control plane | `fcguest/main.go` | fixed | `46ee238` |
| M121 | medium | Unbounded forged background-task notices enable resource exhaustion | `daemon/logtail.go` | fixed | `df082d0` |
| M122 | medium | Unbounded transcript retention can exhaust the operator TUI | `tui/model.go` | fixed | `9d387f0` |
| M123 | medium | Clear RPC ignores guest deletion failures and reports success | `daemon/groups.go` | fixed | `7396dce` |
| M124 | medium | Authenticated wildcard subscriber can permanently allocate tailers for nonexistent groups | `daemon/grpc_server.go` | fixed | `c88f522` |
| M125 | medium | Wildcard-authorized Send can exhaust shared storage across group buckets | `daemon/attachments.go` | fixed | `c88f522` |
| M126 | medium | OAuth login executes an untrusted PATH-selected `claude` binary | `daemon/claude_login.go` | fixed (the check-to-use gap) | `db10fe7` |
| M127 | medium | Shell error text bypasses TUI terminal-output sanitization | `tui/shell_view.go` | fixed | `85a5aee` |
| M128 | medium | State directory validation is not bound to subsequent writes | `daemon/install.go` | not a finding (M52/M77/M112 again) | `85a5aee` |
| M129 | medium | Guest-controlled Venice proxy lacks per-group spend or request-rate enforcement | `daemon/proxy.go` | accepted | `85a5aee` |
| M130 | medium | Goal interruption can fail to cancel a reserved turn | `daemon/goals.go` | fixed | `2c26289` |
| U1 | medium | Stale shell pane accepts input after group selection | `tui/mouse.go` | **open (untriaged)** | — |
| M131 | medium | Untrusted custom-theme filenames reach terminal output without validation | `tui/theme.go` | fixed | `3d4da68` |
| M132 | medium | Daemon can read and replace the TUI's administrator credentials | `tui/daemon.go` | accepted, no change | `b3473ab` |
| M133 | medium | Per-session clear retains named-session notification markers | `daemon/sessions.go` | fixed | `9afd68f` |
| M134 | medium | Stale autostart decisions can resurrect stopped or destroyed groups | `daemon/groups.go` | fixed (destroy half was already closed) | `1e52f30` |
| M135 | medium | Slow request bodies exhaust proxy concurrency slots | `daemon/proxy.go` | fixed | `8264975` |
| M136 | medium | Daemon creates sensitive state with permissive filesystem modes and trusts the configured state root | `daemon/daemon.go` | fixed | `b486eee` |
| M137 | medium | Active detached jobs can be removed from concurrency accounting | `sidecar/cs-job` | already fixed (duplicate of M83) | `d9f8ae7` |
| M138 | medium | Mount failure unconditionally reformats the persistent workspace | `fcguest/main.go` | fixed | `7af64b8` |
| M139 | medium | OAuth login follows an untrusted pre-existing `.claude` symlink | `daemon/claude_login.go` | fixed | `4aa97cd` |
| M140 | medium | Canceled turns cannot leave a full slot pool | `daemon/queue.go` | fixed | `4262ade` |
| M141 | medium | Sanitizer accepts deceptive SGR attributes from untrusted output | `daemon/sanitize.go` | fixed | `54b36c1` |
| M142 | medium | Installer accepts a missing or incomplete artifact manifest and installs unchecked binaries | `daemon/install.go` | fixed | `8dca8f4` |
| M143 | medium | Guest-controlled PTY backpressure can freeze the operator TUI | `tui/shell_view.go` | fixed | `0679d45` |
| M144 | medium | Per-session log clear can exhaust daemon memory | `daemon/sessions.go` | fixed | `aa8f57e` |
| M145 | medium | Failed self-heal advances a session while the stalled guest worker may remain alive | `daemon/send.go` | fixed | `5ad751f` |
| M146 | medium | pki init follows attacker-controlled acl.json symlinks | `daemon/pki.go` | fixed | `e55111b`, `bea54d7` |
| M147 | medium | Credential writes follow attacker-controlled state-directory symlinks | `daemon/setup_steps.go` | fixed | `bea54d7` |
| M148 | medium | Malformed provider configuration fails open to Venice, causing unintended third-party disclosure | `sidecar/cs-subagent` | fixed (the write half was already closed) | `1e06ce9` |
| M149 | medium | Guest can forge notification session attribution and suppress unread routing | `daemon/ctl.go` | fixed | `3941ebb` |
| M150 | medium | Unbounded Claude stream parser state permits guest memory exhaustion | `fcguest/turn.go` | fixed | `a909ef5` |
| M151 | medium | Forced Firecracker stop can race offline workspace-image resize | `daemon/fc.go` | fixed together | `8f8dd28` |
| M152 | medium | Firecracker memory accounting is released before VMM termination | `daemon/fc.go` | fixed together | `8f8dd28` |
| M153 | medium | Default fleet memory cap ignores the daemon's effective cgroup limit | `daemon/fchostmem.go` | fixed | `c51a382` |
| M154 | medium | SubscribeGroup can retain streams indefinitely after subscriber shutdown | `daemon/grpc_server.go` | partly fixed, with the limit stated | `72961ab` |
| M155 | medium | TUI retains deleted group data across clear, destroy, and same-name group reuse | `tui/model.go` | fixed | `cc1b5f3` |
| M156 | medium | SubscribeLogs silently drops audit events under subscriber backpressure | `daemon/events.go` | fixed | `98ce946` |
| M157 | medium | Incomplete Unicode format filtering permits operator-facing text spoofing | `daemon/sanitize.go` | fixed | `848715d` |
| M158 | medium | Minute-by-minute cron evaluation enables authenticated scheduler denial of service | `daemon/cron.go` | fixed | `92a9512` |
| M159 | medium | Group name `host` aliases fleet-wide alert state and can suppress host filesystem notifications | `daemon/resources.go` | fixed | `0df74e6` |
| M160 | medium | Job inspection performs unbounded guest-controlled reads and buffers responses | `daemon/jobs.go` | fixed | `9140d70` |
| M161 | medium | Detached worker descendants can hold stderr open and stall turn cleanup | `fcguest/turn.go` | fixed | `d5bf719` |
| M162 | medium | Guest-controlled event payloads can cause excessive daemon memory use through unbudgeted parsing and replay retention | `daemon/events.go` | fixed (the parser half was already closed) | `cba5b09` |
| M163 | medium | Workspace images have no fleet-wide host-disk admission control | `daemon/fc.go` | fixed | `3f5462f` |
| M164 | medium | Untrusted job output is parsed as authenticated daemon framing | `daemon/grpc_server.go` | fixed | `726b06a` |
| M165 | medium | OAuth login inherits a caller-controlled credential destination | `daemon/claude_login.go` | fixed | `0df74e6` |
| M166 | medium | JobTail parsed output can exhaust TUI memory through byte-unbounded snapshots | `tui/model.go` | fixed | `4840588` |
| L1 | low | Oversized control line permanently stops guest-to-host forwarding | `fcguest/main.go` | fixed | `f2cba0a` |
| L2 | low | Optimistic pending prompts allow terminal escape-sequence injection | `tui/view.go` | fixed | `4d53829` |
| L3 | low | Special filesystem objects can pin a worker through synchronous file operations | `sidecar/venice_stream.js` | fixed | `f2cba0a` |
| L4 | low | Unprivileged installer replaces unreadable root-only environment file | `daemon/install.go` | fixed | `4d53829` |
| L5 | low | Group-wide clear reports success when transcript truncation fails | `daemon/groups.go` | fixed | `4d53829` |
| L6 | low | Out-of-range proxy ports collapse VM jail identities onto UID 30000 | `daemon/fcjail.go` | fixed | `4d53829` |
| L7 | low | Quadratic work when rendering newline-dense guest text frames | `daemon/fcturn.go` | not A finding | `954cd48` |
| L8 | low | Terminal control-sequence injection via unsanitized TUI input | `tui/view.go` | fixed | `954cd48` |
| L9 | low | Setup instructs operators to disable host-wide AppArmor user-namespace protection | `daemon/setup_checks.go` | fixed | `954cd48` |
| L10 | low | Turn output queue is bounded by frame count, not bytes, enabling guest memory exhaustion | `fcguest/turn.go` | fixed | `954cd48` |
| L11 | low | Host-memory protection fails open when capacity discovery fails | `daemon/fchostmem.go` | fixed | `954cd48` |
| L12 | low | Operator-facing setup hint executes mutable third-party script as root | `daemon/setup_checks.go` | fixed | `954cd48` |
| L13 | low | Stale published listeners survive VM exit and ignore updated port configuration | `daemon/fc.go` | fixed | `954cd48` |
| L14 | low | Persisted conversation state is exposed through permissive filesystem permissions | `daemon/send.go` | already fixed (M136) | `954cd48` |
| L15 | low | Workspace-controlled Bash login startup files enable persistent worker code execution | `sidecar/venice_stream.js` | fixed | `0d54074` |
| L16 | low | Terminal control injection via untrusted library names and script output | `tui/view.go` | fixed | `0d54074` |
| L17 | low | Unchanged credential file can remain world-readable | `daemon/install.go` | fixed | `0d54074` |
| L18 | low | Stale cached transcripts remain renderable after group authorization is removed | `tui/model.go` | fixed | `0d54074` |
| L19 | low | Unredacted tool arguments are persisted in group logs and replayed to transcript readers | `daemon/fcturn.go` | accepted | `0d54074` |
| L20 | low | Invalid or unreadable provider configuration can silently route prompts to Venice while displaying Claude | `daemon/proxy.go` | already fixed (M148) | `0d54074` |
| L21 | low | Shell command injection in peer-facing report recipe | `daemon/report.go` | fixed | `0d54074` |
| L22 | low | Partial-line truncation enables embedded log markers to become semantic events | `daemon/logtail.go` | fixed | `0d54074` |
| L23 | low | Guest-controlled newlines inject forged daemon log records | `daemon/events.go` | fixed | `076fd96` |
| L24 | low | Workspace images are created with insecure permissions | `daemon/fc.go` | already fixed (M136) | `076fd96` |
| L25 | low | Guest control payloads are logged globally without field-aware redaction | `daemon/ctlpb.go` | already fixed (M115) | `076fd96` |
| L26 | low | govulncheck scans the workspace dependency graph instead of each module's build graph | `.github/workflows/govulncheck.yml` | fixed | `076fd96` |
| L27 | low | Resource and alert caches survive same-name group recreation | `daemon/resources.go` | fixed | `076fd96` |
| L28 | low | Guest-controlled control requests can cause bounded daemon and TUI memory exhaustion through oversized log events | `daemon/events.go` | already fixed (M115) | `076fd96` |
| L29 | low | Configuration write failures are silently acknowledged, leaving restrictive posture changes unapplied | `daemon/config.go` | already fixed | `076fd96` |
| L30 | low | Notification timer race allows duplicate autonomous turns | `daemon/notify.go` | fixed | `076fd96` |
| L31 | low | AttachShell retains handlers and vsock resources after guest termination | `daemon/grpc_server.go` | partly fixed, with the limit stated | `076fd96` |
| L32 | low | Unbounded activity-state retention after group destruction | `daemon/activity.go` | fixed | `076fd96` |
| L33 | low | Unvalidated PKI client name enables writes outside the credentials directory | `daemon/pki.go` | fixed | `93a7bcb` |
| L34 | low | Installation copies guest assets with unbounded buffering and destroys sparse-file allocation | `daemon/install.go` | fixed | `93a7bcb` |
| L35 | low | Delivered attachments are retained indefinitely in each guest workspace | `fcguest/main.go` | fixed | `93a7bcb` |
| L36 | low | Recovery mount drops nosuid/nodev protections on the worker-writable workspace | `fcguest/main.go` | already fixed (M138) | `93a7bcb` |
| L37 | low | Per-file log ceiling is bypassable by oversized and concurrent appends | `daemon/fc.go` | fixed | `93a7bcb` |
| L38 | low | Unnormalized multiline input escapes the framed prompt box | `tui/view.go` | fixed | `93a7bcb` |
| L39 | low | History RPC performs uncancellable log replay after client cancellation | `daemon/grpc_server.go` | fixed | `93a7bcb` |
| L40 | low | Warning-only raw RunScript mode permits terminal escape injection | `daemon/ctl_cli.go` | fixed | `93a7bcb` |
| L41 | low | Goal names can shadow goal IDs in control operations | `daemon/goals.go` | fixed | `93a7bcb` |
| L42 | low | Denied `/destroy` leaks the existing subscription and opens a replacement stream | `tui/model.go` | fixed | `93a7bcb` |
| L43 | low | OAuth credential path mismatch can make the daemon use an unintended account | `daemon/proxy.go` | fixed | `1cc0b2b` |
| L44 | low | Authenticated SubscribeLogs clients can retain unbounded subscribers and exhaust daemon resources | `daemon/grpc_server.go` | already fixed (M95, L156) | `1cc0b2b` |
| L45 | low | Unbounded cron representation enables authenticated schedule-store resource exhaustion | `daemon/schedules.go` | fixed | `1cc0b2b` |
| L46 | low | Pending job notification can restart a group after `/stop` | `daemon/notify.go` | fixed | `1cc0b2b` |
| L47 | low | Sequence cursor is reused across daemon/group lifecycles, suppressing the required gap | `daemon/events.go` | fixed | `1cc0b2b` |
| L48 | low | Scheduled occurrences are committed before queue admission and are permanently lost on overflow | `daemon/schedules.go` | fixed | `1cc0b2b` |
| L49 | low | Goal handoff capture replays and sorts every active log stream | `daemon/logtail.go` | accepted | `1cc0b2b` |
| L50 | low | Per-command Bash timeout does not terminate descendant processes | `sidecar/venice_stream.js` | fixed | `1cc0b2b` |
| L51 | low | Concurrent clear can resurrect deleted Venice history | `sidecar/venice_stream.js` | already fixed (M60) | `1cc0b2b` |
| L52 | low | Untrusted PATH controls `du` execution during uninstall | `daemon/uninstall.go` | fixed | `1cc0b2b` |
| L53 | low | Unescaped dynamic values allow TUI debug-log injection | `tui/debuglog.go` | fixed | `329f0e2` |
| L54 | low | Uninstall can purge a running daemon when its fixed unit file is absent | `daemon/uninstall.go` | fixed | `329f0e2` |
| L55 | low | WatchState change detection can suppress changed client-visible job state | `daemon/events.go` | fixed | `329f0e2` |
| L56 | low | History RPC accepts an unbounded positive page size | `daemon/grpc_server.go` | fixed | `329f0e2` |
| L57 | low | Self pause/resume resets a goal's MaxIterations budget | `daemon/goals.go` | fixed | `329f0e2` |
| L58 | low | Unbounded completed-job metadata can exhaust daemon and TUI resources | `daemon/jobs.go` | fixed | `329f0e2` |
| L59 | low | Concurrent schedule additions bypass the per-group quota | `daemon/schedules.go` | fixed | `329f0e2` |
| L60 | low | Network-enabled guest traffic lacks a per-guest daemon resource budget | `daemon/fcnet.go` | fixed | `329f0e2` |
| L61 | low | Global security prompt failure is fail-open | `daemon/prompt.go` | fixed | `329f0e2` |
| L62 | low | Persisted group and session names permit terminal escape injection | `tui/view.go` | fixed | `329f0e2` |
| L63 | low | Malformed extended-background SGR bypasses theme background reassertion | `tui/theme.go` | fixed | `3a7960c` |
| L64 | low | Destroy reports success even when workspace deletion fails | `daemon/groups.go` | fixed | `3a7960c` |
| L65 | low | Existing permissive TUI state files retain broad permissions during overwrite | `tui/persist.go` | fixed | `3a7960c` |
| L66 | low | Unbounded schedule listing can exhaust daemon resources | `daemon/schedules.go` | already fixed (M87), residual accepted | `3a7960c` |
| L67 | low | Queue-full recovery can resurrect a stale report window | `daemon/report.go` | fixed | `3a7960c` |
| L68 | low | Read-write systemd binds satisfy the read-only coverage check | `daemon/claude_login.go` | fixed | `3a7960c` |
| L69 | low | Guest-controlled transcript text can exhaust the operator TUI's rendering resources | `daemon/fcturn.go` | fixed | `3a7960c` |
| L70 | low | JobTail reports an oversized log line as a clean end | `daemon/grpc_server.go` | fixed | `3a7960c` |
| L71 | low | Prompt history leaks prompts across chat sessions within a group | `tui/model.go` | fixed | `3a7960c` |
| L72 | low | Guest-controlled job metadata can forge records by breaking TSV framing | `daemon/jobs.go` | fixed | `3a7960c` |
| L73 | low | TUI width fast path misclassifies combining and wide Unicode runes | `tui/width.go` | fixed | `9f5a364` |
| L74 | low | Attachment staging files are readable by local users in the daemon's Unix access group | `daemon/attachments.go` | already fixed | `9f5a364` |
| L75 | low | Goal text and criteria lack application-level size limits | `daemon/goals.go` | fixed | `9f5a364` |
| L76 | low | Background tail writes bypass per-file serialization and can corrupt slot log framing | `daemon/send.go` | already fixed (L37) | `9f5a364` |
| L77 | low | Startup deletes live Firecracker PID files without reconciliation | `daemon/fc.go` | fixed | `9f5a364` |
| L78 | low | Unbounded synchronous file reads allow per-worker memory exhaustion or blocking on special files | `sidecar/venice_stream.js` | already fixed (L3) | `9f5a364` |
| L79 | low | Guest-controlled resource probe can forge per-group disk and memory telemetry | `daemon/resources.go` | fixed (with a stated residual) | `9f5a364` |
| L80 | low | Authenticated Stop/Destroy requests permanently retain mutexes for arbitrary nonexistent group names | `daemon/groups.go` | fixed | `9f5a364` |
| L81 | low | Ignored transcript append errors allow a guest turn to stall and quarantine its slot | `daemon/fcturn.go` | fixed | `9f5a364` |
| L82 | low | Stale asynchronous history responses can restore cleared or destroyed transcripts | `tui/model.go` | fixed | `9f5a364` |
| L83 | low | Venice credential read errors disclose the daemon's absolute credential path to guests | `daemon/proxy.go` | fixed | `f932ed4` |
| L84 | low | Unbounded guest PTY output can monopolize the attached operator TUI | `tui/shell_view.go` | fixed | `f932ed4` |
| L85 | low | Fuzzy picker performs full-corpus scoring and sorting before applying its limit | `tui/fuzzy.go` | fixed | `f932ed4` |
| L86 | low | Guest-controlled flow-key churn causes log and audit-availability pressure | `daemon/fcnet.go` | fixed | `f932ed4` |
| L87 | low | Config writes create unregistered group directories without quota enforcement | `daemon/config.go` | fixed | `f932ed4` |
| L88 | low | Per-rune tab accounting mismeasures ANSI sequences and grapheme clusters | `tui/width.go` | fixed | `f932ed4` |
| L89 | low | Subordinate-ID validation does not cover the full jail UID/GID allocation band | `daemon/userns.go` | fixed | `f932ed4` |
| L90 | low | Concurrent appends bypass the readTail size bound | `daemon/metrics.go` | fixed | `f932ed4` |
| L91 | low | Existing TUI log permissions are not enforced | `tui/debuglog.go` | fixed | `f932ed4` |
| L92 | low | Authorized Spawn RPCs continue booting microVMs after client cancellation | `daemon/grpc_server.go` | fixed | `f932ed4` |
| L93 | low | Shutdown can miss in-flight Firecracker processes | `daemon/fc.go` | fixed | `0a01d18` |
| L94 | low | RunScript output can exhaust the operator TUI through unbounded buffering and transcript retention | `tui/daemon.go` | fixed | `0a01d18` |
| L95 | low | Unbounded metrics append permits guest-driven disk exhaustion | `daemon/proxy.go` | fixed | `0a01d18` |
| L96 | low | Reset group versions can resurrect cleared transcript data from an in-flight prewarm | `tui/model.go` | fixed | `0a01d18` |
| L97 | low | Diagnostic hint recommends printing the protected API credential | `daemon/claude_login.go` | fixed | `0a01d18` |
| L98 | low | Destroyed groups retain name-keyed notification limiter state | `daemon/groups.go` | fixed | `0a01d18` |
| L99 | low | Halfwidth Katakana sound marks can break the shell pane's width invariant | `tui/vt_scrub.go` | fixed | `0a01d18` |
| L100 | low | Fleet view stale shell hit-testing misroutes operator input | `tui/mouse.go` | fixed | `0a01d18` |
| L101 | low | Go pre-commit validation skips manifest-only changes | `tools/hooks/pre-commit` | fixed | `0a01d18` |
| L102 | low | Custom-theme loading can block the TUI on special files and has no aggregate work limits | `tui/theme.go` | fixed | `0a01d18` |
| L103 | low | Setup diagnostics render attacker-controlled text directly to the terminal | `daemon/setup_ui.go` | fixed | `318c53c` |
| L104 | low | Newline injection forges trusted-looking TUI status rows | `tui/goal_cmds.go` | fixed | `318c53c` |
| L105 | low | Authenticated group-scoped persistent worker denial via unchecked Spawn model | `daemon/grpc_server.go` | fixed | `318c53c` |
| L106 | low | Backup copy of reserved global prompt bypasses fireability guard | `tui/prompts.go` | fixed | `318c53c` |
| L107 | low | Guest-agent error text reaches terminal sinks without sanitization | `daemon/fc.go` | mostly fixed already (M74), residual closed | `318c53c` |
| L108 | low | Protocol changes do not invalidate the cached Firecracker rootfs | `Makefile` | fixed | `318c53c` |
| L109 | low | Slash-command arguments are persisted verbatim in the TUI debug log | `tui/model.go` | fixed | `318c53c` |
| L110 | low | Tracked commands can leak os/exec pipe-copy goroutines and race output collection | `fcguest/main.go` | fixed | `318c53c` |
| L111 | low | IPv6 extension headers and fragmented IPv4 packets can evade summarized egress logging | `daemon/fcnet.go` | fixed | `318c53c` |
| L112 | low | Timestamp-only upload names can collide and overwrite queued attachments | `daemon/attachments.go` | fixed | `318c53c` |
| L113 | low | Unthrottled main-only tail polling consumes daemon host resources | `daemon/ctl.go` | fixed | `a4ec5a5` |
| L114 | low | Count-only Markdown caching can exhaust operator TUI memory | `tui/model.go` | fixed | `a4ec5a5` |
| L115 | low | Concurrent turn completion can hide another in-flight turn from shared activity state | `daemon/activity.go` | fixed | `a4ec5a5` |
| L116 | low | Provider-controlled streaming response can exhaust sidecar memory | `sidecar/venice_stream.js` | fixed | `a4ec5a5` |
| L117 | low | Unvalidated guest filesystem counters can corrupt disk telemetry | `daemon/resources.go` | fixed | `a4ec5a5` |
| L118 | low | Concurrent config materialization can corrupt the shared group configuration | `fcguest/main.go` | fixed | `a4ec5a5` |
| L119 | low | Unbounded concurrent TUI prewarming can exhaust client resources | `tui/model.go` | fixed | `a4ec5a5` |
| L120 | low | Repeated same-kind transcript merging causes quadratic TUI work | `tui/model.go` | fixed | `a4ec5a5` |
| L121 | low | Full send queue can permanently suppress a job completion notification | `daemon/notify.go` | fixed | `a4ec5a5` |
| L122 | low | Unbounded leading-zero SGR parameters permit TUI render resource exhaustion | `tui/sgr.go` | fixed | `a4ec5a5` |
| L123 | low | Release vulnerability scan does not reproduce the CGO-disabled build configuration | `.github/workflows/govulncheck.yml` | fixed | `076fd96` |
| L124 | low | Non-streaming LLM responses are buffered without a response-size limit | `daemon/proxy.go` | fixed | `71c6a6c` |
| L125 | low | Queued guest probes can delay resource threshold evaluation | `daemon/resources.go` | fixed | `71c6a6c` |
| L126 | low | Malformed guest frames can poison egress flow audit records | `daemon/fcnet.go` | fixed | `71c6a6c` |
| L127 | low | AttachShell permits unbounded persistent tmux-session creation | `fcguest/main.go` | fixed | `71c6a6c` |
| L128 | low | Destroyed groups leave log tailers and file descriptors running | `daemon/groups.go` | fixed | `71c6a6c` |
| L129 | low | Unbounded published-port fan-out enables authorized resource exhaustion | `daemon/config.go` | fixed | `71c6a6c` |
| L130 | low | History lacks an aggregate byte budget and can burden the operator TUI | `daemon/logtail.go` | fixed | `71c6a6c` |
| L131 | low | Per-slot log ceilings allow aggregate host-storage exhaustion | `daemon/fc.go` | fixed | `71c6a6c` |
| L132 | low | Per-group proxy semaphore channels are never reclaimed | `daemon/proxy.go` | fixed | `71c6a6c` |
| L133 | low | Concurrent client provisioning can lose token authorization entries | `daemon/pki.go` | fixed | `71c6a6c` |
| L134 | low | Authenticated Resources RPC permits unbounded concurrent fleet-wide scans | `daemon/grpc_server.go` | fixed | `39579ce` |
| L135 | low | Unbounded prompt history can exhaust TUI resources when the picker is opened or filtered | `tui/view.go` | fixed | `39579ce` |
| L136 | low | VM exit and startup-failure paths leave per-VM cgroups behind | `daemon/fc.go` | fixed | `39579ce` |
| L137 | low | Partial CA state silently replaces the trust anchor and causes an mTLS outage | `daemon/pki.go` | fixed | `39579ce` |
| L138 | low | Reserved goal-session LLM calls contaminate group activity state | `daemon/proxy.go` | mostly fixed by L115, residual accepted | `39579ce` |
| L139 | low | Host disconnect can permanently stall guest network redial | `fcguest/net.go` | fixed | `39579ce` |
| L140 | low | Malformed history is treated as empty and then overwrites the recoverable transcript | `sidecar/venice_stream.js` | fixed | `39579ce` |
| L141 | low | Tool payloads can grow the TUI job transcript without an aggregate byte limit | `tui/model.go` | fixed | `39579ce` |
| L142 | low | Live and queued rows allow raw tabs to corrupt terminal layout | `tui/view.go` | fixed | `39579ce` |
| L143 | low | `clean` can delete jobs during initialization or before completion notification | `sidecar/cs-job` | fixed | `39579ce` |
| L144 | low | PKI initialization fails to renew expired server certificates | `daemon/pki.go` | fixed | `70b2d16` |
| L145 | low | Container fallback can scan the wrong Git index | `tools/hooks/pre-commit` | fixed | `70b2d16` |
| L146 | low | SIGTERM/SIGINT can bypass VM cleanup during daemon startup | `daemon/daemon.go` | fixed | `70b2d16` |
| L147 | low | Independent error buckets can exhaust the shared main notification queue | `daemon/logalert.go` | fixed | `70b2d16` |
| L148 | low | Deadline-less streaming RPC setup can stall TUI reconnects | `tui/daemon.go` | partly already covered, watchdog added | `70b2d16` |
| L149 | low | Null tool arguments terminate the isolated Venice worker | `sidecar/venice_stream.js` | fixed | `70b2d16` |
| L150 | low | Monochrome post-layout folding expands ZWJ grapheme clusters | `tui/mono.go` | fixed | `70b2d16` |
| L151 | low | Unbounded subagent buffers and detached-job output allow tenant-scoped resource exhaustion | `sidecar/stream_filter.js` | fixed (persisted job output accepted) | `70b2d16` |
| L152 | low | TUI library loader permits blocking special-file reads and unbounded local allocation | `tui/scripts.go` | fixed | `70b2d16` |
| L153 | low | Goal enqueue retry ignores cancellation and sleeps after the final failure | `daemon/goals.go` | fixed | `70b2d16` |
| L154 | low | SubscribeLogs can reorder live events across concurrent producers | `daemon/events.go` | fixed | `75e5930` |
| L155 | low | Notification parser decodes oversized fields before enforcing caps | `daemon/logparse.go` | fixed | `75e5930` |
| L156 | low | Time-dependent tree hit-test uses a stale row index | `tui/mouse.go` | fixed | `75e5930` |
| L157 | low | Guest job metadata can impose repeated daemon allocation and hashing work | `daemon/events.go` | already fixed (M160, L58) | `75e5930` |
| L158 | low | KOTO_FC_NOJAIL does not bypass the daemon's unconditional user-namespace bootstrap | `daemon/daemon.go` | fixed | `75e5930` |
| L159 | low | Unbounded prompt and suggestion text can exhaust the TUI renderer | `tui/view.go` | fixed | `75e5930` |
| L160 | low | Prewarm command races on the shared session map | `tui/model.go` | fixed | `75e5930` |
| L161 | low | Destroyed groups leave unbounded jobsPrimed entries in the TUI | `tui/model.go` | fixed | `75e5930` |
| L162 | low | Guest-writable job metadata can terminate `cs-job peek -new` | `sidecar/cs-job` | fixed | `75e5930` |
| L163 | low | Unbounded stdin ingestion allows ctl client memory exhaustion | `daemon/ctl_cli.go` | fixed | `75e5930` |
| L164 | low | Unbounded persisted TUI state can exhaust resources before startup | `tui/persist.go` | fixed | `75e5930` |
| L165 | low | Strict mtime watermark can permanently strand pending attachments | `daemon/fc.go` | already fixed (M36) | `75e5930` |
| L166 | low | `ctlShell` can terminate while leaving the operator's terminal in raw mode | `daemon/ctl_cli.go` | fixed | `75e5930` |
| L167 | low | Stream closure leaves stale activity state in the TUI | `tui/model.go` | fixed | `75e5930` |
| L168 | low | Fixed `.disabled` destination can be silently overwritten during credential cleanup | `daemon/claude_login.go` | fixed | `75e5930` |
| L169 | low | Guest-controlled JobLogs metadata can corrupt framing and output integrity | `daemon/jobs.go` | already fixed (M160) | `75e5930` |
| L170 | low | Interactive OAuth subprocess cannot reliably be cancelled with SIGINT | `daemon/claude_login.go` | fixed | `75e5930` |
| L171 | low | Valid zero MemAvailable readings are discarded and rendered as non-alerting RSS | `tui/wire.go` | fixed | `75e5930` |
